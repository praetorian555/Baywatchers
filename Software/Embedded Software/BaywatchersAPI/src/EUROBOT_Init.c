...
void InitUSART(USART_TypeDef* USARTx, uint16_t Mode, uint32_t BaudRate, uint16_t WordLength, uint16_t StopBits, uint16_t Parity)
{
  ...
  if (USARTx == USART1)
  {
    ...
    InitGPIO_Pin(GPIOA, GPIO_Pin_9, GPIO_Mode_AF_PP, GPIO_Speed_50MHz);          //Linija Tx za USART1.
    InitGPIO_Pin(GPIOA, GPIO_Pin_10, GPIO_Mode_IN_FLOATING, GPIO_Speed_50MHz);   //Linija Rx za USART1.
  }
  if (USARTx == USART2)
  {
    RCC_APB1PeriphClockCmd(RCC_APB1Periph_USART2, ENABLE);
    InitGPIO_Pin(GPIOA, GPIO_Pin_2, GPIO_Mode_AF_PP, GPIO_Speed_50MHz);          //Linija Tx za USART2.
    InitGPIO_Pin(GPIOA, GPIO_Pin_3, GPIO_Mode_IN_FLOATING, GPIO_Speed_50MHz);    //Linija Rx za USART2.
  }
  if (USARTx == USART3)
  {
    RCC_APB1PeriphClockCmd(RCC_APB1Periph_USART3, ENABLE);
    InitGPIO_Pin(GPIOB, GPIO_Pin_10, GPIO_Mode_AF_PP, GPIO_Speed_50MHz);         //Linija Tx za USART3.
    InitGPIO_Pin(GPIOB, GPIO_Pin_11, GPIO_Mode_IN_FLOATING, GPIO_Speed_50MHz);   //Linija Rx za USART3.
  }
